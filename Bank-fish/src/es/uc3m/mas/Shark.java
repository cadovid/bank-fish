...
//A Shark, managed by an agent
public class Shark extends FBObject {
    // Constants
    ...
    protected boolean inAlignment(Shark p) {
        double distanceSquare = distanceSquare(p);
        return (distanceSquare < DISTANCE_MAX_SQUARE && distanceSquare >
                DISTANCE_MIN_SQUARE);
    }
...
// If collision, calculation of the diff vector
    ...
    protected boolean avoidShark(Shark[] SharkList) {
// Find the nearest Shark
        Shark p;
        if (!SharkList[0].equals(this)) {
            p = SharkList[0];
        } else {
            p = SharkList[1];
        }
        ...
        for (Shark Shark : SharkList) {
            if (distanceSquare(Shark) < distanceSquare && !Shark.equals(this)) {
                p = Shark;
...
// Avoidance
    ...
    protected void calculateAverageDirection(Shark[] SharkList) {
        double speedXTotal = 0;
        double speedYTotal = 0;
        int nbTotal = 0;
        for (Shark p : SharkList) {
            if (inAlignment(p)) {
    ...
    protected void update(Shark[] SharkList, ArrayList<AreaToAvoid> obstacles, double width, double height) {
        if (!avoidWalls(0, 0, width, height)) {
            if (!avoidObstacles(obstacles)) {
                if (!avoidShark(SharkList)) {
                    calculateAverageDirection(SharkList);
                }
            }
        }
        updatePosition();
